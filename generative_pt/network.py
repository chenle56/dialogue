...
def model_config():
    ...
    net_arg.add_argument("--embed_size", type=int, default=300)  #embed_size表示将词嵌入到几维的空间,一般设置完后不用再调
    net_arg.add_argument("--hidden_size", type=int, default=800)#两层双向GRU，每层800个隐状态
    net_arg.add_argument("--bidirectional", type=str2bool, default=True)
    net_arg.add_argument("--max_vocab_size", type=int, default=30000)  #词汇量
    ...
    train_arg.add_argument("--optimizer", type=str, default="Adam") #优化器Adam
    train_arg.add_argument("--lr", type=float, default=0.00005) #搜索步长    一般有固定值:优化器Adam:lr=0.001,优化器SGD:lr=0.01
    train_arg.add_argument("--grad_clip", type=float, default=5.0)   #gradient clipping梯度裁剪,防止梯度爆炸
    train_arg.add_argument("--dropout", type=float, default=0.3)     #小训练集时，屏蔽掉某些神经元，防止过拟合，一般认为设置为0.5或者0.3
    train_arg.add_argument("--num_epochs", type=int, default=4)
    train_arg.add_argument("--pretrain_epoch", type=int, default=2)#前五个epoches---→最小化BOW Loss---→对knowledge manager进行预训练
    train_arg.add_argument("--lr_decay", type=float, default=None)#让搜索步长不断减少--→减缓震荡  decay越大，lr衰减越快；decay越小，lr衰减越慢
    train_arg.add_argument("--use_embed", type=str2bool, default=True)
    train_arg.add_argument("--use_bow", type=str2bool, default=True)
    train_arg.add_argument("--use_dssm", type=str2bool, default=False) #深度语义匹配模型
    ...
    train_arg.add_argument("--decode_concat", type=str2bool, default=False)        # k和c
    ...
    # MISC   在Linux驱动中把无法归类的五花八门的设备定义为混杂设备(用miscdevice结构体表述)
    ...
    misc_arg.add_argument("--ckpt", type=str)     #ckpt: 保存checkpoint模型文件
...
def main():
    ...
    model = KnowledgeSeq2Seq(src_vocab_size=corpus.SRC.vocab_size,
                             ...
                             use_bow=config.use_bow, use_dssm=config.use_dssm,  #BoW:第i个元素表示字典中第i个单词在句子中出现的次数    dssm:计算文本相似度
                             use_pg=config.use_pg, use_gs=config.use_gs,
                             pretrain_epoch=config.pretrain_epoch,
                             use_posterior=config.use_posterior,        #后验
                             weight_control=config.weight_control,
                             concat=config.decode_concat)
    model_name = model.__class__.__name__
                 #model_name'KnowledgeSeq2Seq'
    ...
    # Testing
        ...
        #Adam (amsgrad: False    betas: (0.9, 0.999)    eps: 1e-08   lr: 5e-05    weight_decay: 0)
