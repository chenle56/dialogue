...
class RNNEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.rnn = nn.GRU(input_size=self.input_size,     #Gated Recurrent Unit:lstm的一个变体      gate_size = 3 * hidden_size
    ...
    def forward(self, inputs, hidden=None):
        ...
        if lengths is not None:
            ...
            """
            Packs a Tensor containing padded sequences of variable length,Input can be of size ``T x B x *``,`T` is the length of the longest sequence,`B` is the batch size, and `*` is any number of dimensions
            """
            rnn_inputs = pack_padded_sequence(
                rnn_inputs[:num_valid],
                sorted_lengths[:num_valid].tolist(),#将数组或者矩阵转换成列表
                batch_first=True)
        ...
        if lengths is not None:
            ...
            if num_valid < batch_size:
                zeros = outputs.new_zeros(    #Returns a Tensor
                    batch_size - num_valid, outputs.size(1), self.hidden_size)
                outputs = torch.cat([outputs, zeros], dim=0) #-> Tensor
            ...
            outputs = outputs.index_select(0, inv_indices)  # -> Tensor
