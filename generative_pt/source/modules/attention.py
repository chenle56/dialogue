...
"""
Attention mechanism:
编解码器框架内，通过在编码段加入Attention模型，对源数据序列进行数据加权变换
在解码端引入Attention 模型，对目标数据进行加权变化，有效提高序列对序列的自然方式下的系统表现。
从两个角度来分类 Attention种类： 空间注意力 Spatial Attention,时间注意力 Temporal Attention
从 Attention的作用方法上:Soft Attention 和 Hard Attention
"""
class Attention(nn.Module):
    """
    Attention:__init__,__repr__,forward,
    """
    def __init__(self,
                 query_size,
                 memory_size=None,
    ...
    def forward(self, query, memory, mask=None):
        ...
        if self.mode == "dot":
            ...
            """torch.bmm是批矩阵乘操作,attention里将encoder的输出和attention权值相乘,bmm: 权重*向量,得到attention向量 """
        ...
        if self.project:
            project_output = self.linear_project(
                torch.cat([weighted_memory, query], dim=-1))         # torch.cat用于粘贴,dim=-1指dim1方向粘贴
