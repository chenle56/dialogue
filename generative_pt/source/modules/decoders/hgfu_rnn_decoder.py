...
class RNNDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc1 = nn.Linear(self.hidden_size, self.hidden_size)      #Linear(in_features=800, out_features=800, bias=True)
        self.fc2 = nn.Linear(self.hidden_size, self.hidden_size)      #Linear(in_features=800, out_features=800, bias=True)
        if self.concat:
            self.fc3 = nn.Linear(self.hidden_size * 2, self.hidden_size)
        else:
            self.fc3 = nn.Linear(self.hidden_size * 2, 1)   #Linear(in_features=1600, out_features=1, bias=True)
        self.tanh = nn.Tanh()
        ...
        if self.out_input_size > self.hidden_size:
            self.output_layer = nn.Sequential(
                ...
                nn.Linear(self.out_input_size, self.hidden_size),    #第一层
                nn.Linear(self.hidden_size, self.output_size),       #第二层
                nn.LogSoftmax(dim=-1),                       # 最后一层是log_softmax()
            )
        else:
            self.output_layer = nn.Sequential(
                nn.Dropout(p=self.dropout),
