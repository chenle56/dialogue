...
class Pack(dict):
    ...
    def add(self, **kwargs):   # **表示接受一个dict类型（字典）  k:v
        """
        add
        """
    ...
    def flatten(self):    #降维  &  拼接   =then=》 返回一个一维数组
        """
        flatten
        a = [[1,2],[3,4]]   a.flatten()   a = [1,2,3,4]
        """
        pack_list = []
...
def sequence_mask(lengths, max_len=None):
    ...
    mask = mask.repeat(1, *lengths.size(), 1)# *表示接受一个tuple类型（元组）
    mask = mask.squeeze(0)
