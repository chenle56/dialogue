...
class KnowledgeSeq2Seq(BaseModel):
    """
    KnowledgeSeq2Seq:__init__,encode,decode,forward,collect_metrics,iterate
    ...
    def __init__(self, src_vocab_size, tgt_vocab_size, embed_size, hidden_size, padding_idx=None,
        ...
        if self.with_bridge:
            self.bridge = nn.Sequential(nn.Linear(self.hidden_size, self.hidden_size), nn.Tanh())                            #定义一个linear层 ,通过Squential将网络层和激活函数结合起来，输出激活后的网络节点
        ...
        self.kl_loss = torch.nn.KLDivLoss(size_average=True)        #KLDivLoss()   #select related background knowledge -> lead the conversation
    ...
    def encode(self, inputs, hidden=None, is_training=False):
        """
        encode
        pack():__getattr__,add,flatten,cuda,
        """
        outputs = Pack()
